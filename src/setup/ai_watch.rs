    ...
    async fn run_ai_watch_service_setup_install_check_and_remove_round_trip() {
        ...
        assert!(
            install
                .phases
                .iter()
                .any(|phase| phase.name == "ai-watch-service-files"
        ...
        assert!(
            install
                .phases
                .iter()
                .any(|phase| phase.name == "ai-watch-initial-index"
                    && phase.status == SetupStatus::Ok)
        );
        assert!(
            install.phases.iter().any(
                |phase| phase.name == AI_WATCH_SERVICE_ACTIVE_PHASE && phase.detail == "active"
            )
        );
        ...
        assert!(
            check
                .phases
                .iter()
                .any(|phase| phase.name == AI_WATCH_SERVICE_ENABLED_PHASE
                    && phase.detail == "enabled")
        );
