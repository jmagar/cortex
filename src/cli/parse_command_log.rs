...
fn parse_shell_user_command(args: &[String]) -> Result<ShellUserCommand> {
    ...
    match command.as_str() {
        ...
        "atuinindex" => parse_shell_atuin_index(rest),
        _ => bail!(
            "{}",
...
fn parse_shell_index(args: &[String]) -> Result<ShellUserCommand> {
    ...
    let path = path.ok_or_else(|| anyhow::anyhow!("shell index requires a path"))?;
    if shell != "zsh" {
...
fn parse_shell_atuin_index(args: &[String]) -> Result<ShellUserCommand> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--json" => json = true,
            other if !other.starts_with('-') && path.is_none() => path = Some(other.to_string()),
            other => bail!("unknown shell atuinindex argument: {other}"),
        }
        i += 1;
    }
...
fn parse_shell_agent_index(args: &[String]) -> Result<ShellAgentCommand> {
    ...
    let path = path.ok_or_else(|| anyhow::anyhow!("shell agent index requires a path"))?;
...
fn parse_shell_agent_wrap(args: &[String]) -> Result<ShellAgentCommand> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            other if !other.starts_with('-') && spool.is_none() => spool = Some(other.to_string()),
    ...
    let spool = spool.ok_or_else(|| anyhow::anyhow!("shell agent wrap requires a spool path"))?;
    let start =
