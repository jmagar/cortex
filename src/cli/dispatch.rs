...
use super::{
    ...
    AiListArgs, AiPruneCheckpointsArgs, AiSearchArgs, AiSimilarArgs, AiWatchArgs, AnomaliesArgs,
    AppsArgs, CliMode, ClockSkewArgs, CompareArgs, CorrelateArgs, DbBackupArgs, DbCheckpointArgs,
    DbIntegrityArgs, DbStatusArgs, DbVacuumArgs, IncidentArgs, IngestRateArgs, NotifyRecentArgs,
    NotifyTestArgs, OutputArgs, PatternsArgs, SearchArgs, SessionsArgs, SigAckArgs, SigListArgs,
    SigUnackArgs, SilentHostsArgs, SourceIpsArgs, TailArgs, TimeRangeArgs, TimelineArgs,
};
...
//
// 5 new top-level CLI subcommands wired through the same Local/Http branching
// pattern used by the existing surface-parity handlers (source-ips, timeline,
// patterns, ingest-rate). Each command has matching REST handler in src/api.rs
// and an HTTP client method in src/cli/http_client.rs.
...
impl CompareArgs {
    /// All four time-range flags are required by the service. Surface them
    /// to the operator with a clear, per-flag error instead of silently
    /// posting empty strings.
...
pub(super) async fn run_clock_skew(mode: &CliMode, args: ClockSkewArgs) -> Result<()> {
    let json = args.json;
    let req = args.into_request();
    let response = match mode {
...
pub(super) async fn run_anomalies(mode: &CliMode, args: AnomaliesArgs) -> Result<()> {
    let json = args.json;
    let req = args.into_request();
    let response = match mode {
...
pub(super) async fn run_apps(mode: &CliMode, args: AppsArgs) -> Result<()> {
    let json = args.json;
    let req = args.into_request();
    let response = match mode {
