use anyhow::Result;
use cortex::app::{AiSkillIncidentResponse, AiSkillInvestigateResponse};

use super::super::super::color::{cyan, muted, primary, severity, violet, warn};
use super::super::common::{local_ts, print_json};

pub(crate) fn print_ai_skill_incidents_response(
    response: &AiSkillIncidentResponse,
    json: bool,
) -> Result<()> {
    if json {
    ...
    println!(
        ...
        if response.candidate_window_truncated {
            format!(
                "\n{}: candidate scan capped at {} rows; narrow with --skill/--plugin/--since/--until",
                warn("warning"),
...
pub(crate) fn print_ai_skill_investigate_response(
    response: &AiSkillInvestigateResponse,
    json: bool,
) -> Result<()> {
    if json {
        return print_json(response);
    ...
    if response.no_data {
        println!(
            "No skill events found for the given filter.\nTry: {}",
    ...
    for ev in &response.evidence {
        ...
        println!();
        println!(
            "incident {} [{}] skill={} tool={} project={} session={}",
            primary(&inc.incident_id),
            severity(&inc.priority_label),
            primary(&inc.skill_name),
            violet(&inc.tool),
        ...
        println!(
            "  {} skill event(s), {} signal anchor(s), {} transcript-before, {} transcript-after, \
             {} nearby log(s), {} nearby error(s)",
            cyan(&ev.skill_events.len().to_string()),
