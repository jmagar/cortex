...
async fn stdio_client(
    ...
    let binary = env!("CARGO_BIN_EXE_cortex");
    let (transport, stderr) = TokioChildProcess::builder(Command::new(binary).configure(|cmd| {
        cmd.arg("mcp")
            .env("CORTEX_DB_PATH", db_path)
            .env("CORTEX_RETENTION_DAYS", "0")
            .env("CORTEX_MAX_DB_SIZE_MB", "0")
            .env("CORTEX_RECOVERY_DB_SIZE_MB", "0")
            .env("CORTEX_MIN_FREE_DISK_MB", "0")
            .env("CORTEX_RECOVERY_FREE_DISK_MB", "0")
            .env("RUST_LOG", "warn")
            .env_remove("CORTEX_TOKEN")
...
async fn stdio_child_process_lists_tools_and_calls_queries() {
    ...
    let stats = service
        .call_tool(
            CallToolRequestParams::new("cortex")
    ...
    let search = service
        .call_tool(
            CallToolRequestParams::new("cortex").with_arguments(
