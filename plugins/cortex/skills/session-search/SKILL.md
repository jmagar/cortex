...
Expands skill-usage incidents into deterministic evidence bundles. Accepts a skill name directly. Use for "what went wrong with troubleshoot" or "show me skill failure incidents".
...
```
mcp__cortex__cortex(action="skill_investigate", skill="troubleshoot", limit=5)
```
...
# Investigate skill failures
cortex sessions investigate --skill troubleshoot --limit 5 --json
...
async () => {
  const skills = ["troubleshoot", "redeploy", "logs"];
