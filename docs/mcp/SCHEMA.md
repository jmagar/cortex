...
| `file_tails` | `op` is required and enumerated as `list|add|remove|enable|disable|status`; add requires `id`, `path`, and `tag`; remove/enable/disable require `id`; optional `hostname`, `facility`, `severity`, `start_at_end` |
