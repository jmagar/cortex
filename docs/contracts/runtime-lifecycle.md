...
4. **OAuth env triple and admin email set** when `auth.mode = oauth`: `SYSLOG_MCP_PUBLIC_URL`, `SYSLOG_MCP_GOOGLE_CLIENT_ID`, `SYSLOG_MCP_GOOGLE_CLIENT_SECRET`, plus `SYSLOG_MCP_AUTH_ADMIN_EMAIL` or `mcp.auth.admin_email`. Non-empty `mcp.auth.allowed_emails` is rejected until lab-auth enforces it.
