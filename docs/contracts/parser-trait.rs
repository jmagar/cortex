...
//! swag, adguard, fail2ban) MUST conform to these types. Changes to these
//! types ripple through every parser and the dispatcher — treat this file as
//! API.
//!
//! ## Compilation
//!
//! This file is intentionally a self-contained module. It compiles cleanly
//! against a crate that has `serde`, `serde_json`, `thiserror`, and (optional)
//! `tracing` available. `thiserror` is not yet in `Cargo.toml` at the time
//! this contract was written — add it as a dependency before dropping the
...
//!
//! When implementers begin coding, copy the types into `src/enrich/parser.rs`
//! and keep this file as the human-readable reference. The dispatcher in
//! `src/enrich/dispatch.rs` and every parser module under `src/enrich/parsers/`
